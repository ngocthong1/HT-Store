...
import { useCartCunsumer } from '../../provider/cartProvider';
  ...
  return (
    <div className="flex text-white justify-between items-center">
      {cartState.addedProducts.find(
        ...
        <>
          <button className={' h-8 w-8 button'} onClick={increaseHandler}>
            {' '}
            +{' '}
          </button>
          ...
          {item.quantity > 1 ? (
            <button className={'h-8 w-8 button'} onClick={decreaseHandler}>
              {' '}
              -{' '}
            </button>
          ) : (
            <button
              className={'h-8 w-8 button !bg-red-500'}
        ...
        <button className="h-8 px-4 button" onClick={addHandler}>
          {' '}
          Add To Cart{' '}
        </button>
      )}
